...
<DevicePreview
  ios={<AvatarPreview platform="ios" />}
  android={<AvatarPreview platform="android" />}
/>

## Installation

```bash
npx @codervigi/rn-shad-cli@latest add avatar
```
...
- **Smart Fallbacks**: Automatically generates initials from full names (e.g., "John Doe" → "JD")
